...
/**
 * Hover-driven testimonial row. Each avatar, on hover, plays an optional voice
 * clip and "types out" its quote in a tooltip bubble.
 *
 * @param {{
 *   testimonials: Array<{
 *     image: string;     // avatar URL
 *     audio?: string;    // file name in /public/audio/ (optional)
 *     text: string;      // quote, typed out on hover
 *     name: string;
 *     jobtitle: string;
 *   }>
 * }} props
...
export const Component = ({ testimonials }) => {
  ...
  const handleMouseEnter = useCallback(
    (index) => {
      ...
      // Audio is optional — only attempt playback when a clip is provided so a
      // missing /public/audio/* file doesn't spam the console on every hover.
      const clip = testimonials[index].audio;
      if (clip) {
        const newAudio = new Audio(`/audio/${clip}`);
